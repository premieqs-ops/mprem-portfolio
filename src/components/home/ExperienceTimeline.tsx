      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          {experiences.map((exp, i) => (
            <motion.div key={exp.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="relative pl-12 md:pl-0 md:grid md:grid-cols-2 gap-8 mb-12 last:mb-0">
              ...
              <div className={i % 2 === 0 ? "md:text-right md:pr-12" : "md:col-start-2 md:pl-12"}>
                <div className="glass-card p-6 inline-block text-left w-full md:w-auto min-w-[280px]">
                  <p className="text-xs text-accent-light font-medium mb-1">{exp.current ? "Present" : exp.endDate || "\u2014"}{exp.startDate && ` \u00b7 ${exp.startDate}`}</p>
        ...
        <div className="text-center mt-12">
          <Link href="/experience" className="inline-flex items-center gap-2 text-accent-light font-medium hover:gap-3 transition-all">View full experience <ArrowRight className="w-4 h-4" /></Link>
        </div>
      </div>
    </section>
  );
