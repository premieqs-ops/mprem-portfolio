    ...
    <div className="min-h-screen bg-[#05070a] text-white flex">
      <aside className="w-56 border-r border-white/10 bg-[#0a0e14] hidden md:flex flex-col shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {TABS.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
        ...
        <div className="p-3 border-t border-white/10 space-y-1">
          ...
          <button onClick={logout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-white/50 hover:text-red-400">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-white/10 flex items-center justify-between px-4 md:px-6 bg-[#0a0e14]/80 backdrop-blur sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportJson} className="p-2 rounded-lg text-white/40 hover:text-white hover:bg-white/5" title="Export"><Download className="w-4 h-4" /></button>
            <button onClick={() => fileRef.current?.click()} className="p-2 rounded-lg text-white/40 hover:text-white hover:bg-white/5" title="Import"><Upload className="w-4 h-4" /></button>
            <input ref={fileRef} type="file" accept=".json" className="hidden" onChange={async (e) => {
              const f = e.target.files?.[0];
              if (f) { await importJson(f); showToast("Imported"); }
            }} />
            <button onClick={() => { if (confirm("Reset all to defaults?")) { reset(); showToast("Reset done"); } }} className="p-2 rounded-lg text-white/40 hover:text-red-400" title="Reset"><RotateCcw className="w-4 h-4" /></button>
            <button onClick={handleSave} className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium ${
              hasChanges ? "bg-blue-500 text-white shadow-lg shadow-blue-500/25" : "bg-white/10 text-white/50"
            }`}>
        ...
        {toast && (
          <div className="fixed top-20 right-6 z-50 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-sm shadow-xl">
              ...
              {data.services.map((svc, idx) => (
                <div key={svc.id} className="rounded-xl border border-white/10 bg-[#0a0e14] p-5 space-y-3">
                  <div className="flex justify-between"><span className="text-xs text-white/30">#{idx + 1}</span>
                    <button onClick={() => update({ services: data.services.filter((s) => s.id !== svc.id) })} className="text-white/30 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
            ...
            <div className="max-w-3xl space-y-4">
              {data.experiences.map((exp, idx) => (
                <div key={exp.id} className="rounded-xl border border-white/10 bg-[#0a0e14] p-5 space-y-3">
                  <div className="flex justify-between"><span className="text-xs text-white/30">#{idx + 1}</span>
                    <button onClick={() => update({ experiences: data.experiences.filter((e) => e.id !== exp.id) })} className="text-white/30 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ...
                {data.skills.map((sk) => (
                  <div key={sk.id} className="rounded-xl border border-white/10 bg-[#0a0e14] p-4 space-y-2">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => update({ skills: data.skills.filter((s) => s.id !== sk.id) })} className="text-white/30 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                    </div>
              ...
              {data.projects.map((p) => (
                <div key={p.id} className="rounded-xl border border-white/10 bg-[#0a0e14] p-5 space-y-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => update({ projects: data.projects.filter((x) => x.id !== p.id) })} className="text-white/30 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
              ...
              {data.caseStudies.map((cs) => (
                <div key={cs.id} className="rounded-xl border border-white/10 bg-[#0a0e14] p-5 space-y-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => update({ caseStudies: data.caseStudies.filter((x) => x.id !== cs.id) })} className="text-white/30 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </div>
